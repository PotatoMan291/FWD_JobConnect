...
import { openEntrevistaForm } from './entrevistas-form.js';
...
const createBtn = document.getElementById('create-entrevista-btn');
...
async function loadData() {
  ...
  const res = await entrevistasService.getAll({
  ...
  renderTable({
    ...
    data: res.data,
    onEdit: (id, item) => {
      openEntrevistaForm({ item, onSave: loadData });
    },
    onDelete: (id, item) => {
...
function openDeleteConfirmation(id, item) {
  ...
  overlay.querySelector('.confirm-delete-btn').addEventListener('click', async () => {
    const res = await entrevistasService.remove(id);
    if (res.ok) {
...
createBtn.addEventListener('click', () => {
  openEntrevistaForm({ onSave: loadData });
});
